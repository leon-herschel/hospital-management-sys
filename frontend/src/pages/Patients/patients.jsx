...
function PatientVisualDashboard() {
    ...
    const unsubscribePatients = onValue(patientRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (isAdminUser()) {
          ...
          // Regular users only see patients from their affiliated clinic
          filteredPatients = allPatients.filter(
            (patient) => patient.clinicAffiliation === getUserClinicId()
          );
        }
  ...
  const filteredPatients = patientList.filter((patient) => {
    ...
    const matchesClinicFilter = selectedClinicFilter === "all" || patient.clinicAffiliation === selectedClinicFilter;
  ...
  const patientsByClinic = filteredPatients.reduce((acc, patient) => {
    const clinicId = patient.clinicAffiliation || 'Patients';
  ...
  const uniqueClinics = isAdminUser() ? new Set(filteredPatients.map(p => p.clinicAffiliation)).size : 1;
                    ...
                    <div className="space-y-1 mb-4">
                      ...
                      {isAdminUser() && (
                        <p className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">
                          🏥 {clinics[patient.clinicAffiliation]?.name || patient.clinicAffiliation}
                        </p>
                      )}
                    </div>
            ...
            <table className="w-full text-md text-gray-900">
              <thead className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                <tr>
                  ...
                  {isAdminUser() && <th className="px-6 py-4 text-left font-semibold">Clinic</th>}
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredPatients.length > 0 ? (
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm">
                            <div className="font-medium text-gray-900">
                              {clinics[patient.clinicAffiliation]?.name || 'Unknown Clinic'}
                            </div>
                            <div className="text-gray-500 text-xs">{patient.clinicAffiliation}</div>
                          </div>
                        </td>
                      )}
                  ...
                  <tr>
                    <td colSpan={isAdminUser() ? "5" : "4"} className="px-6 py-8 text-center text-gray-500">
