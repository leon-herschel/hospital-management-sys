...
const ViewBill = ({ billing, patients, onClose }) => {
  ...
  const handleGenerateReceipt = () => {
    ...
    medicineList.forEach((item) => {
      ...
      doc.text(
        `- ${item.itemName} (Quantity: ${item.quantity}, Price: ${item.retailPrice.toFixed(2)} pesos, Subtotal: ${itemSubtotal.toFixed(2)}) pesos`,
        10,
        y,
      );
    ...
    doc.text(
      `Total Amount: ${typeof totalAmount === "number" ? totalAmount.toFixed(2) : "N/A"} pesos`,
      10,
      y + 20,
    );
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 max-h-full overflow-y-auto">
        ...
        <p>
          <strong>Patient Name:</strong> {patients[billing.patientId]}
        </p>
        <p>
